...
---- TASK START ----
### Shim helper task: on(poll.vote_changed)

**todo stubs in FE** 0

**Scope**
1. Extend or create **chatSDKShim.ts** so calls matching `on(poll.vote_changed)` resolve.
2. Run a codemod (jscodeshift / sed) to remove **all** matching
   `// TODO backend-wire-up:on(poll.vote_changed)` occurrences.
