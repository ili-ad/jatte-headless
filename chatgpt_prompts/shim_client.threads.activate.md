...
---- TASK START ----
### Shim helper task: client.threads.activate

**todo stubs in FE** 0

**Scope**
1. Extend or create **chatSDKShim.ts** so calls matching `client.threads.activate` resolve.
2. Run a codemod (jscodeshift / sed) to remove **all** matching
   `// TODO backend-wire-up:client.threads.activate` occurrences.
