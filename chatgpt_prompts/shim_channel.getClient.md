...
---- TASK START ----
### Shim helper task: channel.getClient

**todo stubs in FE** 0

**Scope**
1. Extend or create **chatSDKShim.ts** so calls matching `channel.getClient` resolve.
2. Run a codemod (jscodeshift / sed) to remove **all** matching
   `// TODO backend-wire-up:channel.getClient` occurrences.
