...
---- TASK START ----
### Shim helper task: client.reminders.deleteReminder

**todo stubs in FE** 0

**Scope**
1. Extend or create **chatSDKShim.ts** so calls matching `client.reminders.deleteReminder` resolve.
2. Run a codemod (jscodeshift / sed) to remove **all** matching
   `// TODO backend-wire-up:client.reminders.deleteReminder` occurrences.
