...
---- TASK START ----
### Shim helper task: polls.unregisterSubscriptions

**todo stubs in FE** 0

**Scope**
1. Extend or create **chatSDKShim.ts** so calls matching `polls.unregisterSubscriptions` resolve.
2. Run a codemod (jscodeshift / sed) to remove **all** matching
   `// TODO backend-wire-up:polls.unregisterSubscriptions` occurrences.
