...
---- TASK START ----
### Shim helper task: client.queryChannels

**todo stubs in FE** 0

**Scope**
1. Extend or create **chatSDKShim.ts** so calls matching `client.queryChannels` resolve.
2. Run a codemod (jscodeshift / sed) to remove **all** matching
   `// TODO backend-wire-up:client.queryChannels` occurrences.
