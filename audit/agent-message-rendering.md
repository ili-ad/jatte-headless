...
## MessageProps shape and render path (updated)
- `MessageProps` requires a concrete `message: LocalMessage` plus optional handlers/flags; skeleton rows without a message would violate this contract, so overrides should assume they only render real message rows.【F:libs/stream-chat-shim/src/components/Message/types.ts†L19-L108】
- `MessageList` collects `props.Message` into `internalMessageProps` before calling `useMessageListElements`, and `defaultRenderMessages` injects `<Message message={message} {...messageProps} />` for each non-system `RenderedMessage`. This keeps the `message` prop populated when overriding at the list level.【F:libs/stream-chat-shim/src/components/MessageList/MessageList.tsx†L144-L183】【F:libs/stream-chat-shim/src/components/MessageList/renderMessages.tsx†L98-L136】
- `Channel` loads component overrides (including `Message`) into `ComponentContext`, which `MessageList` reads, so a Channel-level `Message` override applies globally (date separators, intro rows still bypass `Message`). Overriding `MessageList` directly confines the custom renderer to actual message rows produced by `renderMessages`.【F:libs/stream-chat-shim/src/components/Channel/Channel.tsx†L1315-L1389】【F:libs/stream-chat-shim/src/components/MessageList/renderMessages.tsx†L98-L136】
