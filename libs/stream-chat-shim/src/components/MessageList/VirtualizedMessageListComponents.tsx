    ...
    const renderedMessages = items
      .map((item) => {
        ...
        return processedMessages[
          calculateItemIndex(item.originalIndex, PREPEND_OFFSET)
        ];
      })
      .filter((msg) => !!msg);
...
export const Item = ({
  context,
  ...props
}: ItemProps & CommonVirtuosoComponentProps) => {
  if (!context) return <></>;
...
export const messageRenderer = (
  ...
  const streamMessageIndex = calculateItemIndex(
    virtuosoIndex,
    numItemsPrepended,
  );
  ...
  const endOfGroup =
    shouldGroupByUser &&
    (message.user?.id !== maybeNextMessage?.user?.id ||
  ...
  const isFirstUnreadMessage = getIsFirstUnreadMessage({
    ...
    lastReadMessageId,
    message,
    previousMessage: streamMessageIndex
