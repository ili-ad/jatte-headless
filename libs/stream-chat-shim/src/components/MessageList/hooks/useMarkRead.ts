...
// import type { Channel, Event, LocalMessage, MessageResponse } from 'stream-chat'; // TODO backend-wire-up
type Channel = any;
type Event = any;
...
const hasReadLastMessage = (channel: Channel, userId: string) => {
  const latestMessageIdInChannel =
    channel.state.latestMessages.slice(-1)[0]?.id;
  const lastReadMessageIdServer =
    channel.state.read[userId]?.last_read_message_id;
...
export const useMarkRead = ({
  ...
  const { markRead, setChannelUnreadUiState } =
