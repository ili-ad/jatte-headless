...
export function usePrependedMessagesCount(
  ...
  const previousFirstMessageOnFirstLoadedPage =
    useRef<RenderedMessage>(undefined);
  ...
  const numItemsPrepended = useMemo(() => {
    ...
    const noNewMessages =
      currentFirstMessage?.id ===
      previousFirstMessageOnFirstLoadedPage.current?.id;
    ...
    for (
      ...
      const messageIsFirstOnFirstLoadedPage =
        messages[prependedMessageCount].id ===
        firstMessageOnFirstLoadedPage.current?.id;

      if (messageIsFirstOnFirstLoadedPage) {
        previousNumItemsPrepended.current =
          prependedMessageCount - firstRealMessageIndex;
        return previousNumItemsPrepended.current;
      }
    }
