# chat_addons/management/commands/ingest_rag_md.py
...
class Command(BaseCommand):
    help = "Ingest markdown docs into DocumentChunk rows for RAG."
    ...
    def add_arguments(self, parser: CommandParser) -> None:
        parser.add_argument(
            ...
            default=None,
            help=(
                "Root folder containing md/ with markdown docs. "
        ...
        parser.add_argument(
            ...
            help="Corpus/state key for these chunks (required if not configured).",
        )
        parser.add_argument(
            "--dry-run",
            action="store_true",
    ...
    def handle(self, *args, **options) -> None:
        ...
        if not state:
            raise CommandError(
                "Missing --state (or AGENT_RAG_STATE). Provide a corpus key to ingest."
            )
        ...
        for md_path in md_files:
            ...
            topic = infer_topic_from_filename(doc_name, state=state)
...
def infer_topic_from_filename(doc_name: str, *, state: str | None = None) -> str:
    """
    ...
    parts = base.split("_")
    if state and parts and parts[0].lower() == state.lower():
        parts = parts[1:]
...
def chunk_section(heading: str, body: str) -> Iterable[Tuple[str, int]]:
    ...
    for para in paragraphs:
        ...
        if para_len >= MAX_CHARS_PER_CHUNK:
            ...
            chunks.append((para, estimate_tokens_from_chars(para)))
            continue
        ...
        if current_len + para_len <= MAX_CHARS_PER_CHUNK:
            current_paras.append(para)
            current_len += para_len
            continue
        ...
        # overlap: repeat last paragraph
        current_paras = [current_paras[-1], para]
        current_len = len(current_paras[0]) + para_len
